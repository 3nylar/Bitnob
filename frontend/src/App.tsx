...
export default function App() {
  ...
  useEffect(() => {
    ...
    function tick() {
      if (!cancelled) void fetchPoolState();
    }

    tick(); // immediate first fetch
    const id = setInterval(tick, POOL_REFRESH_INTERVAL_MS);
    return () => {
      cancelled = true;
      clearInterval(id);
    };
        ...
        <nav className="w-full bg-slate-900/80 backdrop-blur-md border-b border-slate-800/80 sticky top-0 z-50 px-4 sm:px-6 lg:px-8">
          <div className="max-w-7xl mx-auto h-16 flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <div>
                <p className="text-md font-bold tracking-tight text-white leading-none">
                  Practice Hub
                </p>
                <span className="text-[10px] text-slate-500 font-medium font-mono uppercase tracking-wider">
                  AMM Simulator v1.0.0
                </span>
              </div>
            </div>
                  ...
                  <div className="bg-slate-950/60 border border-slate-800 rounded-xl p-4 space-y-2 text-sm">
                    ...
                    {slippageBreached && (
                      <div className="flex items-start gap-1.5 text-amber-400 text-xs pt-1">
                        <AlertTriangle size={14} className="shrink-0 mt-0.5" />
                        <span>
                          This trade may exceed your slippage tolerance before
                          it executes.
                        </span>
                      </div>
                    )}
                ...
                <div className="bg-slate-950/60 border border-slate-800 rounded-2xl p-4">
                  ...
                  {showILExplainer && (
                    <p className="text-xs text-slate-500 mt-3 leading-relaxed">
                      ...
                      Trading fees you've earned as a liquidity provider are
                      baked into the pool's reserves and aren't broken out
                      separately here, so a negative IL doesn't necessarily mean
                      you're worse off overall once fees are counted.
                    </p>
                  )}
                </div>
              </div>
        ...
        <section className="bg-linear-to-b from-slate-900 to-slate-950 border border-slate-800/80 rounded-3xl p-5 sm:p-6 shadow-2xl mt-12">
          <div className="border-b border-slate-800 pb-4 mb-4">
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              How this Automated Market Maker calculates values dynamically in
              real-time
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-sm">
            <div className="space-y-2">
              ...
              <p className="text-slate-400 leading-relaxed text-xs">
                This pool uses Vitalik Buterin's classic equation:{" "}
                <code className="text-blue-400 bg-slate-950 px-1 py-0.5 rounded font-mono">
                  x * y = k
                </code>
                . The total liquidity constant multiplier (
                <code className="text-slate-300 font-mono">k</code>) must remain
                fixed during trades. When you buy Token A, its reserve
                decreases, causing its price to go up automatically to satisfy
                the system formula.
              </p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <p className="text-slate-400 leading-relaxed text-xs">
                When adding liquidity, assets must match the exact pre-existing
                ratio of the pool (
                <code className="text-slate-300 font-mono">
                  reserve_b / reserve_a
                </code>
                ). This ensures your deposit changes the size of the pool
                without creating structural price spikes or arbitrage extraction
                paths for trading bots.
              </p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <p className="text-slate-400 leading-relaxed text-xs">
                If the price of assets diverges heavily from the ratio when you
                deposited them, your portfolio balance changes as traders swap
                against the pool. Your total value inside the pool might become
                lower than if you had simply held the individual tokens inside a
                cold-storage wallet.
              </p>
            </div>
          </div>
          ...
          <div className="max-w-5xl mx-auto flex flex-col sm:flex-row items-start justify-between gap-6">
            <div className="max-w-xs">
              ...
              <p className="leading-relaxed">
                A constant product AMM simulator. Swap tokens, provide
                liquidity, and watch impermanent loss play out in real time —
                built with React, TypeScript & Python.
              </p>
            </div>
            <p className="text-slate-600 sm:self-end">
              © Lara 2026. All rights reserved.
            </p>
          </div>
        </footer>
      </div>
